...
/**********************************************************************    Top 50   ****************************************************************************/
		...
		function makeTop50TracksTable(trackData,audioFeatureData){
			...
			{v: energy, f: energy + '%'},{v: valence, f: valence + '%'},{v: danceability, f: danceability + '%'}, more];
		...
		function makePlaylistTable(trackdata,audioFeaturedata){
			...
			for(var i=0;i<tracksObj.items.length;i++){
				...
				var tempArray = [no,{v: title, f: titleString},{v: firstArtist, f: trackArtists},{v: popularity, f: popularity + '%'},
					...
					{v: audioFeature.danceability, f: audioFeature.danceability + '%'},more];
			...
			var statsArray = [["For track " + tracksCovered,{v: popularityTotal, f: popularityTotal + '%'},tempoTotal,{v: energyTotal, f: energyTotal + '%'},
									{v: valenceTotal, f: valenceTotal + '%'},{v: danceabilityTotal, f: danceabilityTotal + '%'}, majorTotal]];				
			google.charts.setOnLoadCallback(drawStatsTrackTable);
			function drawStatsTrackTable() {
				var data = new google.visualization.DataTable();
